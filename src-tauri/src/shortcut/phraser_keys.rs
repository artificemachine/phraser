//! Phraser-keys based keyboard shortcut implementation
//!
//! This module provides an alternative to Tauri's global-shortcut plugin
//! using the phraser-keys library for more control over keyboard events.
//!
//! ## Architecture
//!
...
use phraser_keys::{Hotkey, HotkeyId, HotkeyManager, HotkeyState, KeyboardListener};
use serde::Serialize;
use specta::Type;
...
/// State for the phraser-keys shortcut manager
pub struct PhraserKeysState {
...
impl PhraserKeysState {
    /// Create a new PhraserKeysState
    ...
    fn manager_thread(cmd_rx: Receiver<ManagerCommand>, app: AppHandle) {
        info!("phraser-keys manager thread started");
        ...
        loop {
            ...
            while let Some(event) = manager.try_recv() {
                if let Some((binding_id, hotkey_string)) = hotkey_to_binding.get(&event.id) {
                    debug!(
                        "phraser-keys event: binding={}, hotkey={}, state={:?}",
            ...
            match cmd_rx.recv_timeout(std::time::Duration::from_millis(10)) {
                Ok(cmd) => match cmd {
                    ...
                    ManagerCommand::Shutdown => {
                        info!("phraser-keys manager thread shutting down");
                        break;
                    }
                },
        ...
        info!("phraser-keys manager thread stopped");
    }
    ...
    fn do_register(
        ...
        debug!(
            "Registered phraser-keys shortcut: {} -> {:?}",
            binding_id, hotkey
        );
        Ok(())
    }
    ...
    fn do_unregister(
        ...
        if let Some(id) = binding_to_hotkey.remove(binding_id) {
            ...
            debug!("Unregistered phraser-keys shortcut: {}", binding_id);
        }
        Ok(())
    }
    ...
    pub fn start_recording(&self, app: &AppHandle, binding_id: String) -> Result<(), String> {
        ...
        debug!("Started phraser-keys recording mode");
        Ok(())
    }
    ...
    fn recording_loop(app: AppHandle, running: Arc<AtomicBool>) {
        while running.load(Ordering::SeqCst) {
            let event = {
                let state = match app.try_state::<PhraserKeysState>() {
                    Some(s) => s,
                    None => break,
                };
            ...
            if let Some(key_event) = event {
                ...
                // Emit to frontend
                if let Err(e) = app.emit("phraser-keys-event", &frontend_event) {
    ...
    pub fn stop_recording(&self) -> Result<(), String> {
        ...
        debug!("Stopped phraser-keys recording mode");
        Ok(())
    }
}

impl Drop for PhraserKeysState {
    fn drop(&mut self) {
...
/// Convert phraser-keys Modifiers to a list of strings
fn modifiers_to_strings(modifiers: phraser_keys::Modifiers) -> Vec<String> {
    let mut result = Vec::new();

    if modifiers.contains(phraser_keys::Modifiers::CTRL) {
        result.push("ctrl".to_string());
    }
    if modifiers.contains(phraser_keys::Modifiers::OPT) {
        #[cfg(target_os = "macos")]
        result.push("option".to_string());
        #[cfg(not(target_os = "macos"))]
        result.push("alt".to_string());
    }
    if modifiers.contains(phraser_keys::Modifiers::SHIFT) {
        result.push("shift".to_string());
    }
    if modifiers.contains(phraser_keys::Modifiers::CMD) {
        #[cfg(target_os = "macos")]
        result.push("command".to_string());
        #[cfg(not(target_os = "macos"))]
        result.push("super".to_string());
    }
    if modifiers.contains(phraser_keys::Modifiers::FN) {
...
/// Validate a shortcut string for the PhraserKeys implementation.
/// PhraserKeys is more permissive: allows modifier-only combos and the fn key.
pub fn validate_shortcut(raw: &str) -> Result<(), String> {
    if raw.trim().is_empty() {
        return Err("Shortcut cannot be empty".into());
    }
    // PhraserKeys accepts modifier-only, key-only, and modifier+key combos
    // Just verify the string is parseable
    raw.parse::<Hotkey>()
        .map(|_| ())
        .map_err(|e| format!("Invalid shortcut for PhraserKeys: {}", e))
}

/// Initialize phraser-keys shortcuts
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    let state = PhraserKeysState::new(app.clone())?;
    ...
    for (id, default_binding) in default_bindings {
        ...
        if let Err(e) = state.register(&binding) {
            error!(
                "Failed to register phraser-keys shortcut {} during init: {}",
                id, e
            );
        }
    }
    ...
    app.manage(state);
    info!("phraser-keys shortcuts initialized");
    Ok(())
}
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Some(cancel_binding) = get_settings(&app_clone).bindings.get("cancel").cloned() {
                if let Some(state) = app_clone.try_state::<PhraserKeysState>() {
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    let state = app
        .try_state::<PhraserKeysState>()
        .ok_or("PhraserKeysState not initialized")?;
...
#[tauri::command]
#[specta::specta]
pub fn start_phraser_keys_recording(app: AppHandle, binding_id: String) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::PhraserKeys {
        return Err("phraser-keys is not the active keyboard implementation".into());
    }
...
#[tauri::command]
#[specta::specta]
pub fn stop_phraser_keys_recording(app: AppHandle) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::PhraserKeys {
        return Err("phraser-keys is not the active keyboard implementation".into());
    }
