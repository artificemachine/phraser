...
//! - `phraser_keys`: Uses the phraser-keys library for more control
//!
//! The active implementation is determined by the `keyboard_implementation`
//! setting and can be changed at runtime.

mod handler;
pub mod phraser_keys;
mod tauri_impl;
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhraserKeys => {
            if let Err(e) = phraser_keys::init_shortcuts(app) {
                error!("Failed to initialize phraser-keys shortcuts: {}", e);
                ...
                // Update settings to persist the fallback so we don't retry PhraserKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhraserKeys => phraser_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhraserKeys => phraser_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_action_shortcuts(app: &AppHandle) {
    ...
    for action in &settings.post_process_actions {
        ...
        match settings.keyboard_implementation {
            ...
            KeyboardImplementation::PhraserKeys => {
                phraser_keys::register_action_shortcut(app, binding);
            }
        }
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhraserKeys => phraser_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhraserKeys => phraser_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::PhraserKeys => "phraser_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::PhraserKeys => phraser_keys::validate_shortcut(raw),
    }
}
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        "phraser_keys" => KeyboardImplementation::PhraserKeys,
        other => {
            warn!(
...
/// Initialize PhraserKeys if not already initialized, with rollback on failure
fn initialize_phraser_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<phraser_keys::PhraserKeysState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = phraser_keys::init_shortcuts(app) {
        error!("Failed to initialize PhraserKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize PhraserKeys: {}. Reverted to Tauri.",
            e
        ));
    }
